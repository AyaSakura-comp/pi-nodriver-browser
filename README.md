...
- Click, fill, type, select, keyboard, scroll, wait, text extraction, screenshots, and safe overlay dismissal
...
flowchart LR
    ...
    E -->|JSON lines over Unix socket| W[Persistent worker.py daemon]
    W --> N[Nodriver]
...
- **`worker.py`** — Long-lived Unix-socket daemon that owns the Nodriver browser session and implements browser commands.
...
3. The first command connects to the existing Unix socket or lazily starts `xvfb-run → worker.py --server → Nodriver → Chrome`.
...
7. Pi task/session shutdown closes only its socket connection. The daemon, Xvfb, Chrome, tabs, and in-memory page state remain alive for the next task.
8. Chrome closes only after an explicit `close`, `shutdown`, installer upgrade, uninstall, process termination, or machine restart.
...
| `close` | Explicitly close Chrome while leaving the daemon reusable |
...
- The daemon serializes commands across all connected Pi extension instances.
