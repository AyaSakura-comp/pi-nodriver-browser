...
### 5. Search-to-Image Delivery & Cross-Origin Rendering
* **Source-Agnostic Image Delivery**: After `web_search`, `crawl`, or interactive browser work discovers a direct image URL, the standalone `fetch_image` tool downloads it without requiring or inspecting an open browser page, validates the actual bytes with Pillow, and returns an inline image attachment. It therefore bypasses page vision preflight and quarantine.
* **Implicit Visual Intent**: Tool routing treats appearance questions such as `X 長怎樣？` / `what does X look like?`, plus explicit image/photo requests for visually depictable real-world subjects, as requests to discover and deliver an actual image rather than answer only in prose. Generic requests to show code, calculations, instructions, or other nonvisual content are unaffected. If no suitable direct image can be found, the agent may explain that limitation instead of fabricating one.
* **PiWeb / Discord Transfer**: The result includes a session-isolated local path and the exact `[[image: <path>]]` outbox marker the agent must emit in its final reply, so the user receives the image rather than an inaccessible filesystem link. Saves use exclusive file creation, collision-safe names, and a bounded sanitized filename stem.
...
* **Browser Command Parity**: `fetch-image <url>` exposes the same operation through the `browser` command interface when a workflow is already using that tool.
...
| **`fetch-image` / `fetch_image`** | `fetch-image <http(s)://image-url>` | Fetches and validates a direct image URL, saves it in the session-isolated download directory, and returns an inline image plus a `[[image: <path>]]` delivery marker. `fetch_image` is the standalone tool form. | Session Scope |
...
The current suite contains **226 tests**: 170 fast tests run by default and 56 real-browser tests are skipped unless explicitly enabled.
